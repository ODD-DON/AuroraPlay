...
Item {
    ...
    // Simplified full-screen stream area with top controls
    Rectangle {
        id: streamArea
        anchors.fill: parent
        color: "black"

        // Video placeholder: actual video is provided by native window; keep an overlay for controls
        Rectangle {
            id: videoOverlay
            anchors.fill: parent
            color: "transparent"
            ...
            RowLayout {
                ...
                spacing: 12
                Image { source: "qrc:/icons/auroraplay.svg"; width: 36; height: 36 }
                Label { text: qsTr("AuroraPlay") ; color: "white"; font.pixelSize: 18 }
                Item { Layout.fillWidth: true }
                Button { text: qsTr("Menu"); onClicked: menuController.toggle(); Material.rounded: true }
                Button { text: qsTr("Exit"); onClicked: root.showMainView(); Material.rounded: true }
            }

            // Bottom stats / quick controls
            Item {
                anchors {
                    left: parent.left
                    right: parent.right
