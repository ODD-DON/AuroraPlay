...
extract_zip() {
    ...
    if command -v 7z >/dev/null 2>&1; then
        ...
        echo "ERROR: Missing 7z and unzip. Install one of them in MSYS2 or use a shell with unzip available."
        exit 1
    fi
}

mkdir ninja && cd ninja
wget https://github.com/ninja-build/ninja/releases/download/v1.9.0/ninja-win.zip && extract_zip ninja-win.zip
cd ..
...
wget https://mirror.firedaemon.com/OpenSSL/openssl-1.1.1q.zip && extract_zip openssl-1.1.1q.zip
...
wget https://www.libsdl.org/release/SDL2-devel-2.0.20-VC.zip && extract_zip SDL2-devel-2.0.20-VC.zip
...
wget https://github.com/protocolbuffers/protobuf/releases/download/v3.9.1/protoc-3.9.1-win64.zip && extract_zip protoc-3.9.1-win64.zip
cd ..
