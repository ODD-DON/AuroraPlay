    ...
    Item {
        ColumnLayout {
            ...
            RowLayout {
                ...
                spacing: 12
                Image { source: "qrc:/icons/auroraplay.svg"; width: 40; height: 40 }
                Label { text: qsTr("Settings"); font.pixelSize: 20 }
                Item { Layout.fillWidth: true }
                Button { text: qsTr("Close"); onClicked: dialog.close() }
            }

            // Simple card-based sections for faster navigation
            Flickable {
                id: settingsFlick
                anchors.fill: parent
                contentWidth: Math.max(width, settingsColumn.width)
                ...
                ColumnLayout {
                    ...
                    Rectangle {
                        ...
                        ColumnLayout { anchors.margins: 12; spacing: 8
                            Label { text: qsTr("General"); font.pixelSize: 16 }
                            RowLayout { spacing: 8
                                Label { text: qsTr("Action On Disconnect:") }
                                C.ComboBox { model: [qsTr("Do Nothing"), qsTr("Enter Sleep Mode"), qsTr("Ask")]; currentIndex: Chiaki.settings.disconnectAction; onActivated: index => Chiaki.settings.disconnectAction = index }
                            }
                            RowLayout { spacing: 8
                                Label { text: qsTr("Streamer Mode:") }
                                C.CheckBox { checked: Chiaki.settings.streamerMode; onToggled: Chiaki.settings.streamerMode = !Chiaki.settings.streamerMode }
                            }
                        }
                    }
                    ...
                    Rectangle {
                        ...
                        width: parent.width
                        ColumnLayout { anchors.margins: 12; spacing: 8
                            Label { text: qsTr("Video"); font.pixelSize: 16 }
                            RowLayout { spacing: 8
                                Label { text: qsTr("Hardware Decoder:") }
                                C.ComboBox { model: Chiaki.settings.availableDecoders; currentIndex: Math.max(0, model.indexOf(Chiaki.settings.decoder)); onActivated: (index) => Chiaki.settings.decoder = index ? model[index] : "" }
                            }
                            RowLayout { spacing: 8
                                Label { text: qsTr("Zero-Copy") }
                                C.CheckBox { checked: Chiaki.settings.useZeroCopy; onToggled: Chiaki.settings.useZeroCopy = checked }
                            }
                        }
                    }
                    ...
                    Rectangle {
                        ...
                        width: parent.width
                        ColumnLayout { anchors.margins: 12; spacing: 8
                            Label { text: qsTr("Stream"); font.pixelSize: 16 }
                            RowLayout { spacing: 8
                                Label { text: qsTr("Audio/Video:") }
                                C.ComboBox { model: [qsTr("Audio and Video Enabled"), qsTr("Audio Disabled"), qsTr("Video Disabled"), qsTr("Audio and Video Disabled")]; currentIndex: Chiaki.settings.audioVideoDisabled; onActivated: index => Chiaki.settings.audioVideoDisabled = index }
                            }
                        }
                    }

                    // Small footer note
                    Label { text: qsTr("Advanced settings are available in the original Config tab."); color: Qt.rgba(1,1,1,0.6) }
                }
            }
        }
    }
